...
exports.encodeConnectionHeader = encodeConnectionHeader;
exports.decodeConnectionHeader = decodeConnectionHeader;
    ...
    return new Promise((resolve, reject) => {
        ...
        const scheduleReconnect = () => {
            ...
            if (reconnectAttempts >= maxReconnectAttempts) {
                logger.error(`Max reconnection attempts (${maxReconnectAttempts}) reached. Closing tunnel.`, lastError ? `Last error: ${lastError}` : '');
                close();
                return;
            }
        ...
        const setupWebSocket = () => {
            ...
            const proxyAgent = proxy_transport_1.nodeProxyTransport.getWebSocketAgent(url.toString());
 ...
 */
async function startMultiplexedTcpTunnel(remoteURL, token, hostname, port, options) {
    const logLevel = options?.logLevel ?? 'info';
    const logger = {
        debug: (...args) => {
            if (logLevel === 'debug')
                console.log('[Tunnel:Mux]', ...args);
        },
        info: (...args) => {
            if (logLevel === 'info' || logLevel === 'debug')
                console.log('[Tunnel:Mux]', ...args);
            ...
            return new Promise((resolveWs, rejectWs) => {
                ...
                ws.on('open', () => {
                    ...
                    socket.on('message', (data) => {
                        ...
                        logger.debug(`WS→TCP [conn=${connId}] ${payload.length} bytes: ${payload
 ...
 */
function encodeConnectionHeader(connId) {
 ...
 */
function decodeConnectionHeader(header) {
    if (header.length < 4) {
