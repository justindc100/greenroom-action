...
- A third-party SDK that the test build leaves unconfigured (no key) usually makes no requests; one that is configured with a production key contacts its host and must be either allowlisted (with a test project) or unconfigured in the test build.
