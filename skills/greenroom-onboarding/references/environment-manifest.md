...
| `notes` | string, up to 1000 chars | no | Context for reviewers of the manifest. For a `session_import` manifest the draft records the **account precondition** here ("Account precondition for entryState /(tabs)/today: completed onboarding, free tier"): the server-side state the disposable account must have for a fresh launch to land on the entry state rather than onboarding or a paywall. `--account-state` fills it; until then the draft leaves `REPLACE_WITH_ACCOUNT_STATE` and the check names it. |
