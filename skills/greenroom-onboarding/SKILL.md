...
Read the report and decide four things. Check each against the source before trusting the report; it is a heuristic. It reads the working tree without `node_modules`, build products, `ios/Pods` and hidden directories other than `.github` and `.greenroom` (a nested checkout under `.release-worktrees/` or a tool cache is never the app); an app that lives in a subdirectory needs `--root`.
...
- **Router.** `expo-router` (an `app/` directory), `react-navigation` (one file that declares `<Stack.Screen name=...>`), `swiftui` (the target's source root), or `hash-spa` (one file that defines a `views` object). Anything else has no deterministic extractor; the contract still works through `sources` globs. The report marks routes that only redirect (`app/index.tsx` with a bare `<Redirect>`): they are not screens and the draft leaves them out of `states` and puts their file in `sharedSources`. It also marks routes that **no in-app navigation reaches** ("NO in-app navigation reaches it"): a deep link such as `/invite/:code`, a debug screen nothing pushes, or a route reached by a computed string the scan cannot see. A walk from the entry state cannot get there, so exclude them (`--exclude "/invite/*,/analytics"`) unless one is the entry or you can point at the navigation the scan missed; the draft sorts them last and names them so you decide deliberately.
...
Then run the one for your platform (name only directories the tree has; a missing one makes grep error) for anything the report missed, and check `<link>`/`<script>` tags in HTML for web:

```
grep -rn -E 'fetch\(|axios|WebSocket|XMLHttpRequest' src app        # Expo / React Native
grep -rn -E 'URLSession|NSURLSession' Sources                         # Swift
grep -rn -E 'fetch\(|axios|WebSocket|XMLHttpRequest' src index.html   # web
```
...
Decision: **does the app need a public SDK key to start?** The report lists the key variables the build reads (`sdk-keys`). An SDK the test build leaves unconfigured usually makes no requests; but some SDK initializers throw without a key (RevenueCat's `configure` is one), and then the startup chain never runs and the app shows a blank screen. Read the initializer: if it throws or blocks startup without the key, pass `--sdk-keys <NAME>` so `prepare` reads a **test project's public key** from a GitHub Actions repository variable (`export <NAME>="${{ vars.<FAMILY>_TEST_PUBLIC_KEY }}"` with a `test -n` guard that fails the build early and names the variable; nothing is left to replace in the file, the owner sets the variable), and add the SDK's hosts to `allowedHosts` (RevenueCat: `api.revenuecat.com` and its fallback `api.rc-backup.com`). Public SDK keys (RevenueCat public keys, PostHog project tokens, Stripe publishable keys) ship inside the app binary and may be exported in `prepare` this way; a private key, secret, service-role key or signing key never goes in `prepare`, and `prepare` cannot read repository secrets anyway. If the SDK does not need a key to start, leave the variable unset so the SDK stays off in the test build and its host goes to `productionHosts`.

Decision (signed-in apps): **what server-side state must the account have for the entry state to be reachable?** A disposable account is not enough on its own: read the gated layout's conditions and the app's bootstrap. If the layout also redirects on a server-hydrated flag (completed onboarding, a built plan, a subscription tier, a role), a bare account lands on onboarding or a paywall and the first pass reads as a broken import. Write the answer as one line and pass it as `--account-state "completed onboarding on the staging backend (hasOnboarded true, one built plan), free tier"`: the draft records it in the manifest's `notes` and as the first owner-checklist item, and leaves a `REPLACE_WITH_ACCOUNT_STATE` placeholder that the check names until you do.
...
It prints the `uses:` line the quickstart currently quotes and, on a `release:` line, which runner release that SHA is: inside the Greenroom repository from the trust list, in a customer repository from the release label the public CI-workflow reference (https://docs.getgreenroom.io/docs/reference/ci-workflow) carries beside the pin ("pins runner X.Y.Z (commit ...)"). When neither can be read it prints `release: unknown` and says why; do not guess. The SHA is the identity Greenroom trusts: record the SHA in the setup, and use the release name only to match an issue or changelog written in release numbers. Never copy a SHA from memory or from an old file in the tree; if the docs cannot be read, ask the user to paste the `uses:` line from https://docs.getgreenroom.io/docs/quickstart/ios (or `/web`) and pass it as `--pin`.
...
node "<skill>/scripts/greenroom-onboard.mjs" draft . --out . \
  ...
  [--sdk-keys REVENUECAT_API_KEY_IOS] [--exclude "/legal/*,/modals/*,/invite/*"] [--authenticated "/session,/upgrade"] [--entry "/onboarding"]
  [--account-state "completed onboarding, free tier"] [--bundle-id io.example.app] [--app-name Example] [--scheme Example] [--device "Greenroom QA Example"]
```

`--app` is the app's stable slug in Greenroom (lowercase, `a-z0-9._-`); the owner names the app the same in the Greenroom dashboard. Decide `--exclude` before drafting (legal, modal and diagnostic screens, the routes the report marked as unreachable): `sharedSources` is the intersection of what the **kept** screens import, so an excluded debug route that imports only a database helper no longer stops the theme file every real screen imports from being shared. `detect --exclude "..."` shows the resulting lists before you draft. The draft reads the pin, the framework, the router and the build from step 1 and writes:
...
`pin-cocoapods` reads the lockfile's last line (`COCOAPODS: X.Y.Z`), pins that version in the workflow's `gem install` and `pod _X.Y.Z_ install` lines and removes the recipe comment; it edits nothing else, so your goals, notes and hosts survive. Do not re-run the draft to pin the version: that needs `--overwrite` and discards your hand edits. Commit `.greenroom/Podfile.lock` with the setup. Until the file exists the check reports `podfile-lock-missing`, which is correct.
...
- **Resolve every placeholder you can.** `REPLACE_WITH_ISOLATED_BACKEND_URL` in `prepare` (the build-time backend URL), `REPLACE_WITH_ISOLATED_BACKEND_HOST` in the hosts, `REPLACE_WITH_ACCOUNT_STATE` in the manifest notes (answer it from the gated layout; `--account-state` or a hand edit), `REPLACE_WITH_COCOAPODS_VERSION` (`pin-cocoapods` once the lockfile exists). The check names each one that is left, and only the categories still present. A value only the owner knows (the backend host) stays a placeholder and goes on the checklist; that is expected (section 5). A public SDK key is not a placeholder: `--sdk-keys` reads it from a repository variable the owner sets.
...
Leave the three files in the working tree, uncommitted, with no draft directory or generated `ios/` left behind. Show the user the diff. On a fresh setup every file is untracked, so `git diff` prints nothing: use `git status --short` and show each file's content (`git diff --no-index /dev/null .greenroom/environment.json` renders one as a diff); on an update, `git diff` plus the untracked files. Then the summary:
...
4. The owner checklist the helper printed, in this order:
   - Put the disposable account in the state the entry state needs (signed-in apps only; the `--account-state` answer, repeated from the manifest notes), on the isolated backend the checklist names (the backend the session refreshes against, never the first host in the list); then store the session payload as the repository secret named in the manifest, and the rotation decision if the backend rotates refresh tokens.
   - Set the repository variable named in `prepare` (`<FAMILY>_TEST_PUBLIC_KEY`) to a test project's public key for any SDK the app needs to start (`--sdk-keys`).
   - Produce and commit `.greenroom/Podfile.lock` if the check reported it missing, then run `pin-cocoapods`.
